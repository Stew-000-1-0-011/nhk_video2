...
namespace NhkVideo2
{
	...
	class Omni4Node final : public rclcpp::Node
	{
		...
		void omni4_init_callback(const std_msgs::msg::Empty&)
		{
			...
			#else
			/// @todo 各種設定
			omni_wheel_args[0] = OmniWheelArg{.pose={.point={0.4053625, 0.4053625}, .theta=std::numbers::pi / 4}, .reduction_ratio=1.0, .wheel_radius=0.127 / 2, .base_id=0x160};
			omni_wheel_args[1] = OmniWheelArg{.pose={.point={-0.4053625, 0.4053625}, .theta=std::numbers::pi * 3 / 4}, .reduction_ratio=1.0, .wheel_radius=0.127 / 2, .base_id=0x164};
			omni_wheel_args[2] = OmniWheelArg{.pose={.point={-0.4053625, -0.4053625}, .theta=std::numbers::pi * 5 / 4}, .reduction_ratio=1.0, .wheel_radius=0.127 / 2, .base_id=0x168};
			omni_wheel_args[3] = OmniWheelArg{.pose={.point={0.4053625, -0.4053625}, .theta=std::numbers::pi * 7 / 4}, .reduction_ratio=1.0, .wheel_radius=0.127 / 2, .base_id=0x16C};
			#endif
